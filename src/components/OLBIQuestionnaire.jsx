      ...
      <>
        <Card>
          <div className="text-center py-8">
            ...
            <h3 className="text-heading-2 text-text-primary font-bold mb-2">評估完成</h3>
            <p className="text-body text-text-secondary mb-6">您的評估結果已儲存</p>

            {/* Section Scores */}
            <div className="max-w-2xl mx-auto mb-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="bg-white border border-neutral-200 rounded-lg p-4">
                  <div className="text-sm text-text-tertiary mb-1">{olbiSections.exhaustion.name_zh}</div>
                  <div className="text-2xl font-bold text-primary-600">{exhaustionScore}</div>
                  <div className="text-sm text-text-tertiary mt-1">{olbiSections.exhaustion.description_zh}</div>
                </div>
                <div className="bg-white border border-neutral-200 rounded-lg p-4">
                  <div className="text-sm text-text-tertiary mb-1">{olbiSections.disengagement.name_zh}</div>
                  <div className="text-2xl font-bold text-warning-600">{disengagementScore}</div>
                  <div className="text-sm text-text-tertiary mt-1">{olbiSections.disengagement.description_zh}</div>
                </div>
              </div>
            ...
            <button
              onClick={clearData}
              className="px-6 py-2.5 rounded-lg text-sm font-medium text-error-600 bg-error-50 hover:bg-error-100 border border-error-200 hover:border-error-300 transition-all duration-200"
            >
              清除並重新填寫
            </button>
          </div>
  ...
  return (
    <Card>
      <div className="max-w-4xl mx-auto">
        <h2 className="text-heading-2 text-text-primary font-bold mb-2">Oldenburg Burnout Inventory (OLBI)</h2>
        ...
        <div className="mb-6">
          ...
          <select
            ...
            className="w-full px-4 py-2.5 border border-neutral-300 rounded-lg text-text-primary bg-white focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all duration-200"
          >
        ...
        {Object.entries(olbiSections).map(([sectionId, section]) => {
          ...
          return (
            <div key={sectionId} className="mb-4 border border-neutral-200 rounded-lg overflow-hidden">
              {/* Section Header */}
              <button
                onClick={() => toggleSection(sectionId)}
                className="w-full px-6 py-4 bg-neutral-50 hover:bg-neutral-100 flex items-center justify-between transition-colors duration-200"
              >
                <div className="text-left">
                  <h3 className="text-sm font-semibold text-text-primary">{section.name_zh}</h3>
                  <p className="text-sm text-text-tertiary mt-1">{section.description_zh}</p>
                </div>
              ...
              {isExpanded && (
                <div className="p-6 space-y-6 bg-white">
                  {sectionQuestions.map((question, idx) => {
                    ...
                    return (
                      <div key={question.id} className="pb-6 border-b border-neutral-100 last:border-b-0">
                        <div className="flex items-start gap-2 mb-3">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="text-sm text-text-tertiary italic mt-1">
                              {question.text}
                            </div>
                          </div>
                        </div>
                          ...
                          {question.options.map((option) => (
                            ...
                            >
                              ...
                              <div className="ml-3 flex-1">
                                <span className="text-sm text-text-primary">{option.label_zh}</span>
                                <span className="ml-2 text-sm text-text-tertiary italic">({option.label})</span>
                              </div>
                            </label>
                          ))}
                        </div>
        ...
        <div className="border-t border-neutral-200 pt-6 mt-6">
          <h4 className="text-sm font-semibold text-text-primary mb-3">學術來源與參考文獻</h4>
